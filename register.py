...
def register():
    if entryfirstname.get() == '' or entrylastname.get() == '' or entryemail.get() == '' or entrycontact.get() == '' or \
        ...
        try:
            con = pymysql.connect(host='localhost', user='root', password='rutu@1997', database='register')
            cur = con.cursor()
            ...
            if row != None:
                ...
                cur.execute(
                    'insert into student (f_name,l_name,email,contact,question,answer,password) values(%s,%s,%s,%s,%s,%s,%s)',
